...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for cpxstats availability")
	if infos, err := checkCpxstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["cpxstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["cpxstats"] = infos.Report()
		cpxstats = infos.config
	}
